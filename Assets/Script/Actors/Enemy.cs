...
{
    public class Enemy : AutoMoveCollisionActor
    {
        void Start()
        {
        ...
        protected override void m_updateAfterCommandExecution()
        {
            ...
            if (deletionRegistance - m_calculateOffScreenDeleteEvaluationValue() <= 0)
            {
                Destroy(gameObject);
            }

            scheduler.EnqueueCommand(new SetVariable<int>("deletionResistance", deletionRegistance));
        }
